...
void loop() {
  ...
  cnt++;
  sprintf(message,"%u",cnt);message[strlen(message)]='*';
  time_beg = micros();
...
//Serial.print("...transmitted...");
  ...
  long readTo=0;
  stat=0;
  while(((stat & (1 << RX_DR))==0) && (readTo>=0) ){
    regRead(STATUS,&stat,1);
    readTo=10000-(micros() - time_beg);
  }
  CE_LOW
  
  if(readTo<0){cntko++;Serial.print(cntko);Serial.println(" time out");flushRx();}
  else{
...
//nrfTime0=0;
//timeStat(&nrfTime0,&nrfTime0,&stat,"pwrUpTx ");
//regRead(CONFIG,&conf,1);Serial.print(" conf=");Serial.println(conf,HEX);
...
//timeStat(&nrfTime0,&nrfTime2,&stat,"TxData  ");
